...
function setLogLevel(level) {
  ...
  for (const logger23 of registeredLoggers) {
    if (shouldEnable(logger23)) {
      enabledNamespaces2.push(logger23.namespace);
    }
  }
...
function createLogger(parent, level) {
  const logger23 = Object.assign(parent.extend(level), {
    level
  });
  patchLogMethod(parent, logger23);
  if (shouldEnable(logger23)) {
    const enabledNamespaces2 = debug_default.disable();
    debug_default.enable(enabledNamespaces2 + "," + logger23.namespace);
  }
  registeredLoggers.add(logger23);
  return logger23;
}
function shouldEnable(logger23) {
  return Boolean(azureLogLevel && levelMap[logger23.level] <= levelMap[azureLogLevel]);
}
...
var logger = createClientLogger("identity");
...
function getAliasesFromStaticSources(staticAuthorityOptions, logger23) {
  var _a3;
  let staticAliases;
  const canonicalAuthority = staticAuthorityOptions.canonicalAuthority;
  if (canonicalAuthority) {
    const authorityHost = new UrlString(canonicalAuthority).getUrlComponents().HostNameAndPort;
    staticAliases = getAliasesFromMetadata(authorityHost, (_a3 = staticAuthorityOptions.cloudDiscoveryMetadata) == null ? void 0 : _a3.metadata, AuthorityMetadataSource.CONFIG, logger23) || getAliasesFromMetadata(authorityHost, InstanceDiscoveryMetadata.metadata, AuthorityMetadataSource.HARDCODED_VALUES, logger23) || staticAuthorityOptions.knownAuthorities;
  }
  return staticAliases || [];
}
function getAliasesFromMetadata(authorityHost, cloudDiscoveryMetadata, source, logger23) {
  logger23 == null ? void 0 : logger23.trace(`getAliasesFromMetadata called with source: ${source}`);
  if (authorityHost && cloudDiscoveryMetadata) {
    const metadata = getCloudDiscoveryMetadataFromNetworkResponse(cloudDiscoveryMetadata, authorityHost);
    if (metadata) {
      logger23 == null ? void 0 : logger23.trace(`getAliasesFromMetadata: found cloud discovery metadata in ${source}, returning aliases`);
      return metadata.aliases;
    } else {
      logger23 == null ? void 0 : logger23.trace(`getAliasesFromMetadata: did not find cloud discovery metadata in ${source}`);
    }
  }
  return null;
}
...
var invoke = (callback, eventName, logger23, telemetryClient, correlationId) => {
  return (...args) => {
    logger23.trace(`Executing function ${eventName}`);
    ...
    try {
      ...
      logger23.trace(`Returning result from ${eventName}`);
      return result;
    } catch (e) {
      logger23.trace(`Error occurred in ${eventName}`);
      try {
        logger23.trace(JSON.stringify(e));
      } catch (e2) {
        logger23.trace("Unable to print error message.");
      }
      inProgressEvent == null ? void 0 : inProgressEvent.end({
        success: false
      }, e);
...
var invokeAsync = (callback, eventName, logger23, telemetryClient, correlationId) => {
  return (...args) => {
    logger23.trace(`Executing function ${eventName}`);
    const inProgressEvent = telemetryClient == null ? void 0 : telemetryClient.startMeasurement(eventName, correlationId);
    if (correlationId) {
...
var RegionDiscovery = class _RegionDiscovery {
  constructor(networkInterface, logger23, performanceClient, correlationId) {
...
var Authority = class _Authority {
  constructor(authority, networkInterface, cacheManager, authorityOptions, logger23, correlationId, performanceClient, managedIdentity) {
...
// node_modules/@azure/msal-common/dist/authority/AuthorityFactory.mjs
async function createDiscoveredInstance(authorityUri, networkClient, cacheManager, authorityOptions, logger23, correlationId, performanceClient) {
  ...
  const acquireTokenAuthority = new Authority(authorityUriFinal, networkClient, cacheManager, authorityOptions, logger23, correlationId, performanceClient);
  try {
    await invokeAsync(acquireTokenAuthority.resolveEndpointsAsync.bind(acquireTokenAuthority), PerformanceEvents.AuthorityResolveEndpointsAsync, logger23, performanceClient, correlationId)();
...
var AccountEntity = class _AccountEntity {
   ...
   * @param authType
   */
  static generateHomeAccountId(serverClientInfo, authType, logger23, cryptoObj, idTokenClaims) {
    if (!(authType === AuthorityType.Adfs || authType === AuthorityType.Dsts)) {
      ...
      logger23.warning("No client info in response");
    }
...
var CacheError = class _CacheError extends Error {
  constructor(errorCode, errorMessage2) {
    const message = errorMessage2 || (CacheErrorMessages[errorCode] ? CacheErrorMessages[errorCode] : CacheErrorMessages[cacheUnknownErrorCode]);
...
var CacheManager = class _CacheManager {
  constructor(clientId, cryptoImpl, logger23, staticAuthorityOptions) {
    this.clientId = clientId;
    this.cryptoImpl = cryptoImpl;
    this.commonLogger = logger23.clone(name, version);
    ...
    try {
      ...
      if (e instanceof Error) {
        ...
        if (e.name === "QuotaExceededError" || e.name === "NS_ERROR_DOM_QUOTA_REACHED" || e.message.includes("exceeded the quota")) {
          (_c2 = this.commonLogger) == null ? void 0 : _c2.error(`CacheManager.saveCacheRecord: exceeded storage quota`, correlationId);
          throw new CacheError(cacheQuotaExceededErrorCode);
        } else {
          throw new CacheError(e.name, e.message);
        }
      } else {
        (_d2 = this.commonLogger) == null ? void 0 : _d2.errorPii(`CacheManager.saveCacheRecord: ${e}`, correlationId);
   ...
   * @returns
   */
  updateOutdatedCachedAccount(accountKey, accountEntity, logger23) {
    var _a3;
    if (accountEntity && accountEntity.isSingleTenant()) {
      (_a3 = this.commonLogger) == null ? void 0 : _a3.verbose("updateOutdatedCachedAccount: Found a single-tenant (outdated) account entity in the cache, migrating to multi-tenant account entity");
      ...
      logger23 == null ? void 0 : logger23.verbose("Updated an outdated account entity in the cache");
...
var BaseClient = class {
  ...
  async executePostToTokenEndpoint(tokenEndpoint, queryString, headers, thumbprint, correlationId, queuedEvent) {
    ...
    (_d2 = this.performanceClient) == null ? void 0 : _d2.addFields({
      refreshTokenSize: ((_b2 = response.body.refresh_token) == null ? void 0 : _b2.length) || 0,
      httpVerToken: ((_c2 = response.headers) == null ? void 0 : _c2[HeaderNames.X_MS_HTTP_VERSION]) || ""
    }, correlationId);
...
var PopTokenGenerator = class {
   ...
   * @returns
   */
  async generateCnf(request, logger23) {
    var _a3;
    ...
    const reqCnf = await invokeAsync(this.generateKid.bind(this), PerformanceEvents.PopTokenGenerateCnf, logger23, this.performanceClient, request.correlationId)(request);
...
var ResponseHandler = class _ResponseHandler {
  constructor(clientId, cacheStorage, cryptoObj, logger23, serializableCache, persistencePlugin, performanceClient) {
    this.clientId = clientId;
...
function buildAccountToCache(cacheStorage, authority, homeAccountId, base64Decode2, idTokenClaims, clientInfo, environment, claimsTenantId, authCodePayload, nativeAccountId, logger23) {
  logger23 == null ? void 0 : logger23.verbose("setCachedAccount called");
  ...
  if (baseAccountKey) {
    cachedAccount = cacheStorage.getAccount(baseAccountKey, logger23);
  }
...
var AuthorizationCodeClient = class extends BaseClient {
  ...
  async getAuthCodeUrl(request) {
    var _a3;
    (_a3 = this.performanceClient) == null ? void 0 : _a3.addQueueMeasurement(PerformanceEvents.GetAuthCodeUrl, request.correlationId);
  ...
  async createAuthCodeUrlQueryString(request) {
    var _a3, _b2;
    (_a3 = this.performanceClient) == null ? void 0 : _a3.addQueueMeasurement(PerformanceEvents.AuthClientCreateQueryString, request.correlationId);
    const parameterBuilder = new RequestParameterBuilder();
...
var RefreshTokenClient = class extends BaseClient {
  ...
  async createTokenRequestBody(request) {
    ...
    parameterBuilder.addScopes(request.scopes, true, (_c2 = this.config.authOptions.authority.options.OIDCOptions) == null ? void 0 : _c2.defaultScopes);
...
function addError(error, logger23, event, stackMaxSize = 5) {
  var _a3, _b2;
  if (!(error instanceof Error)) {
    logger23.trace("PerformanceClient.addErrorStack: Input error is not instance of Error", event.correlationId);
    return;
    ...
    logger23.trace("PerformanceClient.addErrorStack: Stack already exist", event.correlationId);
    return;
  } else if (!((_b2 = error.stack) == null ? void 0 : _b2.length)) {
    logger23.trace("PerformanceClient.addErrorStack: Input stack is empty", event.correlationId);
    return;
  }
  if (error.stack) {
...
var PerformanceClient = class {
   ...
   */
  constructor(clientId, authority, logger23, libraryName, libraryVersion, applicationTelemetry, intFields, abbreviations) {
...
function validateCryptoAvailable(logger23) {
  if ("crypto" in window) {
    logger23.verbose("BrowserCrypto: modern crypto interface available");
  } else {
    logger23.error("BrowserCrypto: crypto interface is unavailable");
...
function buildConfiguration({ auth: userInputAuth, cache: userInputCache, system: userInputSystem, telemetry: userInputTelemetry }, isBrowserEnvironment) {
  ...
  if ((userInputAuth == null ? void 0 : userInputAuth.protocolMode) !== ProtocolMode.OIDC && (userInputAuth == null ? void 0 : userInputAuth.OIDCOptions)) {
    const logger23 = new Logger(providedSystemOptions.loggerOptions);
    logger23.warning(JSON.stringify(createClientConfigurationError(ClientConfigurationErrorCodes_exports.cannotSetOIDCOptions)));
  }
...
var CryptoOps = class _CryptoOps {
  constructor(logger23, performanceClient) {
    this.logger = logger23;
    validateCryptoAvailable(logger23);
...
var BrowserCacheManager = class extends CacheManager {
  constructor(clientId, cacheConfig, cryptoImpl, logger23, staticAuthorityOptions, performanceClient) {
    super(clientId, cryptoImpl, logger23, staticAuthorityOptions);
   ...
   * @param accountKey
   */
  getAccount(accountKey, logger23) {
    this.logger.trace("BrowserCacheManager.getAccount called");
    const accountEntity = this.getCachedAccountEntity(accountKey);
    return this.updateOutdatedCachedAccount(accountKey, accountEntity, logger23);
  }
  /**
  ...
  getCachedAuthority(cachedState) {
    ...
    const state3 = this.getTemporaryCache(stateCacheKey);
    if (!state3) {
      return null;
    }
    const authorityCacheKey = this.generateAuthorityKey(state3);
   ...
   * @param account
   */
  updateCacheEntries(state3, nonce, authorityInstance, loginHint, account) {
    this.logger.trace("BrowserCacheManager.updateCacheEntries called");
    const stateCacheKey = this.generateStateKey(state3);
    this.setTemporaryCache(stateCacheKey, state3, false);
    const nonceCacheKey = this.generateNonceKey(state3);
   ...
   * @param state
   */
  resetRequestCache(state3) {
    this.logger.trace("BrowserCacheManager.resetRequestCache called");
    if (state3) {
      this.temporaryCacheStorage.getKeys().forEach((key) => {
        if (key.indexOf(state3) !== -1) {
          this.removeTemporaryItem(key);
        }
      });
      this.removeTemporaryItem(this.generateStateKey(state3));
      this.removeTemporaryItem(this.generateNonceKey(state3));
      this.removeTemporaryItem(this.generateAuthorityKey(state3));
    }
...
var DEFAULT_BROWSER_CACHE_MANAGER = (clientId, logger23) => {
  const cacheOptions = {
  ...
  return new BrowserCacheManager(clientId, cacheOptions, DEFAULT_CRYPTO_IMPLEMENTATION, logger23);
};

// node_modules/@azure/msal-browser/dist/cache/AccountManager.mjs
function getAllAccounts(logger23, browserStorage, isInBrowser, accountFilter) {
  logger23.verbose("getAllAccounts called");
  return isInBrowser ? browserStorage.getAllAccounts(accountFilter) : [];
}
function getAccount(accountFilter, logger23, browserStorage) {
  logger23.trace("getAccount called");
  if (Object.keys(accountFilter).length === 0) {
    logger23.warning("getAccount: No accountFilter provided");
    return null;
  }
  const account = browserStorage.getAccountInfoFilteredBy(accountFilter);
  if (account) {
    logger23.verbose("getAccount: Account matching provided filter found, returning");
    return account;
  } else {
    logger23.verbose("getAccount: No matching account found, returning null");
    return null;
  }
}
function getAccountByUsername(username, logger23, browserStorage) {
  logger23.trace("getAccountByUsername called");
  if (!username) {
    logger23.warning("getAccountByUsername: No username provided");
    return null;
  }
  ...
  if (account) {
    logger23.verbose("getAccountByUsername: Account matching username found, returning");
    logger23.verbosePii(`getAccountByUsername: Returning signed-in accounts matching username: ${username}`);
    return account;
  } else {
    logger23.verbose("getAccountByUsername: No matching account found, returning null");
    return null;
  }
}
function getAccountByHomeId(homeAccountId, logger23, browserStorage) {
  logger23.trace("getAccountByHomeId called");
  if (!homeAccountId) {
    logger23.warning("getAccountByHomeId: No homeAccountId provided");
    return null;
  }
  const account = browserStorage.getAccountInfoFilteredBy({
    homeAccountId
  ...
  if (account) {
    logger23.verbose("getAccountByHomeId: Account matching homeAccountId found, returning");
    logger23.verbosePii(`getAccountByHomeId: Returning signed-in accounts matching homeAccountId: ${homeAccountId}`);
    return account;
  } else {
    logger23.verbose("getAccountByHomeId: No matching account found, returning null");
    return null;
  }
}
function getAccountByLocalId(localAccountId, logger23, browserStorage) {
  logger23.trace("getAccountByLocalId called");
  if (!localAccountId) {
    logger23.warning("getAccountByLocalId: No localAccountId provided");
    return null;
  }
  const account = browserStorage.getAccountInfoFilteredBy({
    localAccountId
  ...
  if (account) {
    logger23.verbose("getAccountByLocalId: Account matching localAccountId found, returning");
    logger23.verbosePii(`getAccountByLocalId: Returning signed-in accounts matching localAccountId: ${localAccountId}`);
    return account;
  } else {
    logger23.verbose("getAccountByLocalId: No matching account found, returning null");
    return null;
  }
}
...
var EventHandler = class {
  constructor(logger23, browserCrypto) {
  ...
  handleAccountCacheChange(e) {
    var _a3;
    try {
      if ((_a3 = e.key) == null ? void 0 : _a3.includes(PersistentCacheKeys.ACTIVE_ACCOUNT_FILTERS)) {
...
var BaseInteractionClient = class {
  constructor(config, storageImpl, browserCrypto, logger23, eventHandler, navigationClient, performanceClient, nativeMessageHandler, correlationId) {
    this.config = config;
    ...
    this.logger = logger23.clone(BrowserConstants.MSAL_SKU, version2, this.correlationId);
    this.performanceClient = performanceClient;
  }
...
async function generatePkceCodes(performanceClient, logger23, correlationId) {
  performanceClient.addQueueMeasurement(PerformanceEvents.GeneratePkceCodes, correlationId);
  const codeVerifier = invoke(generateCodeVerifier, PerformanceEvents.GenerateCodeVerifier, logger23, performanceClient, correlationId)(performanceClient, logger23, correlationId);
  const codeChallenge = await invokeAsync(generateCodeChallengeFromVerifier, PerformanceEvents.GenerateCodeChallengeFromVerifier, logger23, performanceClient, correlationId)(codeVerifier, performanceClient, logger23, correlationId);
  return {
    verifier: codeVerifier,
    challenge: codeChallenge
  };
}
function generateCodeVerifier(performanceClient, logger23, correlationId) {
  try {
    const buffer = new Uint8Array(RANDOM_BYTE_ARR_LENGTH);
    invoke(getRandomValues, PerformanceEvents.GetRandomValues, logger23, performanceClient, correlationId)(buffer);
...
async function generateCodeChallengeFromVerifier(pkceCodeVerifier, performanceClient, logger23, correlationId) {
  performanceClient.addQueueMeasurement(PerformanceEvents.GenerateCodeChallengeFromVerifier, correlationId);
  try {
    const pkceHashedCodeVerifier = await invokeAsync(sha256Digest, PerformanceEvents.Sha256Digest, logger23, performanceClient, correlationId)(pkceCodeVerifier, performanceClient, correlationId);
...
async function initializeBaseRequest(request, config, performanceClient, logger23) {
  ...
  if (!validatedRequest.authenticationScheme) {
    ...
    logger23.verbose(`Authentication Scheme wasn't explicitly set in request, defaulting to "Bearer" request`);
  } else {
    ...
    logger23.verbose(`Authentication Scheme set to "${validatedRequest.authenticationScheme}" as configured in Auth request`);
  }
...
async function initializeSilentRequest(request, account, config, performanceClient, logger23) {
  performanceClient.addQueueMeasurement(PerformanceEvents.InitializeSilentRequest, request.correlationId);
  const baseRequest = await invokeAsync(initializeBaseRequest, PerformanceEvents.InitializeBaseRequest, logger23, performanceClient, request.correlationId)(request, config, performanceClient, logger23);
  return {
    ...request,
    ...baseRequest,
    account,
...
var StandardInteractionClient = class extends BaseInteractionClient {
  ...
  async getClientConfiguration(serverTelemetryManager, requestAuthority, requestAzureCloudOptions, account) {
    ...
    const logger23 = this.config.system.loggerOptions;
    return {
      authOptions: {
        clientId: this.config.auth.clientId,
      ...
      loggerOptions: {
        loggerCallback: logger23.loggerCallback,
        piiLoggingEnabled: logger23.piiLoggingEnabled,
        logLevel: logger23.logLevel,
        correlationId: this.correlationId
      },
      cacheOptions: {
...
var NativeInteractionClient = class extends BaseInteractionClient {
  constructor(config, browserStorage, browserCrypto, logger23, eventHandler, navigationClient, apiId, performanceClient, provider, accountId, nativeStorageImpl, correlationId) {
    var _a3;
    super(config, browserStorage, browserCrypto, logger23, eventHandler, navigationClient, performanceClient, provider, correlationId);
    this.apiId = apiId;
    this.accountId = accountId;
    this.nativeMessageHandler = provider;
    this.nativeStorageManager = nativeStorageImpl;
    this.silentCacheClient = new SilentCacheClient(config, this.nativeStorageManager, browserCrypto, logger23, eventHandler, navigationClient, performanceClient, provider, correlationId);
    this.serverTelemetryManager = this.initializeServerTelemetryManager(this.apiId);
    const extensionName = this.nativeMessageHandler.getExtensionId() === NativeConstants.PREFERRED_EXTENSION_ID ? "chrome" : ((_a3 = this.nativeMessageHandler.getExtensionId()) == null ? void 0 : _a3.length) ? "unknown" : void 0;
...
var NativeMessageHandler = class _NativeMessageHandler {
  constructor(logger23, handshakeTimeoutMs, performanceClient, extensionId) {
    this.logger = logger23;
   ...
   */
  static async createProvider(logger23, handshakeTimeoutMs, performanceClient) {
    logger23.trace("NativeMessageHandler - createProvider called.");
    try {
      const preferredProvider = new _NativeMessageHandler(logger23, handshakeTimeoutMs, performanceClient, NativeConstants.PREFERRED_EXTENSION_ID);
      await preferredProvider.sendHandshakeRequest();
      return preferredProvider;
    } catch (e) {
      const backupProvider = new _NativeMessageHandler(logger23, handshakeTimeoutMs, performanceClient);
   ...
   */
  static isNativeAvailable(config, logger23, nativeExtensionProvider, authenticationScheme) {
    logger23.trace("isNativeAvailable called");
    if (!config.system.allowNativeBroker) {
      logger23.trace("isNativeAvailable: allowNativeBroker is not enabled, returning false");
      return false;
    }
    if (!nativeExtensionProvider) {
      logger23.trace("isNativeAvailable: WAM extension provider is not initialized, returning false");
      return false;
    }
    if (authenticationScheme) {
      switch (authenticationScheme) {
        ...
        case AuthenticationScheme.POP:
          logger23.trace("isNativeAvailable: authenticationScheme is supported, returning true");
          return true;
        default:
          logger23.trace("isNativeAvailable: authenticationScheme is not supported, returning false");
          return false;
      }
    }
    return true;
...
var InteractionHandler = class {
  constructor(authCodeModule, storageImpl, authCodeRequest, logger23, performanceClient) {
...
function deserializeResponse(responseString, responseLocation, logger23) {
  ...
  if (!serverParams) {
    if (!UrlUtils_exports.stripLeadingHashOrQuery(responseString)) {
      logger23.error(`The request has returned to the redirectUri but a ${responseLocation} is not present. It's likely that the ${responseLocation} has been removed or the page has been redirected by code running on the redirectUri page.`);
      throw createBrowserAuthError(hashEmptyError);
    } else {
      logger23.error(`A ${responseLocation} is present in the iframe but it does not contain known properties. It's likely that the ${responseLocation} has been replaced by code running on the redirectUri page.`);
      logger23.errorPii(`The ${responseLocation} detected is: ${responseString}`);
...
var PopupClient = class extends StandardInteractionClient {
  constructor(config, storageImpl, browserCrypto, logger23, eventHandler, navigationClient, performanceClient, nativeStorageImpl, nativeMessageHandler, correlationId) {
    super(config, storageImpl, browserCrypto, logger23, eventHandler, navigationClient, performanceClient, nativeMessageHandler, correlationId);
    ...
    try {
      ...
      try {
        ...
        if (((_a3 = validRequest.account) == null ? void 0 : _a3.homeAccountId) && validRequest.postLogoutRedirectUri && authClient.authority.protocolMode === ProtocolMode.OIDC) {
          void this.browserStorage.removeAccount((_b2 = validRequest.account) == null ? void 0 : _b2.homeAccountId);
...
var RedirectHandler = class {
   ...
   * @param hash
   */
  async handleCodeResponse(response, state3) {
    this.logger.verbose("RedirectHandler.handleCodeResponse called");
    this.browserStorage.setInteractionInProgress(false);
    const stateKey = this.browserStorage.generateStateKey(state3);
    ...
    this.browserStorage.cleanRequestByState(state3);
    return tokenResponse;
  }
  /**
...
var RedirectClient = class extends StandardInteractionClient {
  ...
  async handleResponse(serverParams, serverTelemetryManager) {
    ...
    const cachedRequest = this.browserStorage.getCachedRequest(state3);
    ...
    if (serverParams.accountId) {
      ...
      const { userRequestState } = ProtocolUtils.parseRequestState(this.browserCrypto, state3);
    ...
    const currentAuthority = this.browserStorage.getCachedAuthority(state3);
    if (!currentAuthority) {
      throw createBrowserAuthError(noCachedAuthorityError);
    }
    ...
    return interactionHandler.handleCodeResponse(serverParams, state3);
  }
  /**
      ...
      if (authClient.authority.protocolMode === ProtocolMode.OIDC) {
        try {
          ...
          if ((_a3 = validLogoutRequest.account) == null ? void 0 : _a3.homeAccountId) {
            void this.browserStorage.removeAccount((_b2 = validLogoutRequest.account) == null ? void 0 : _b2.homeAccountId);
...
async function initiateAuthRequest(requestUrl, performanceClient, logger23, correlationId, navigateFrameWait) {
  performanceClient.addQueueMeasurement(PerformanceEvents.SilentHandlerInitiateAuthRequest, correlationId);
  if (!requestUrl) {
    logger23.info("Navigate url is empty");
    throw createBrowserAuthError(emptyNavigateUri);
  }
  if (navigateFrameWait) {
    return invokeAsync(loadFrame, PerformanceEvents.SilentHandlerLoadFrame, logger23, performanceClient, correlationId)(requestUrl, navigateFrameWait, performanceClient, correlationId);
  }
  return invoke(loadFrameSync, PerformanceEvents.SilentHandlerLoadFrameSync, logger23, performanceClient, correlationId)(requestUrl);
}
async function monitorIframeForHash(iframe, timeout, pollIntervalMilliseconds, performanceClient, logger23, correlationId, responseType) {
  performanceClient.addQueueMeasurement(PerformanceEvents.SilentHandlerMonitorIframeForHash, correlationId);
  return new Promise((resolve, reject) => {
    if (timeout < DEFAULT_IFRAME_TIMEOUT_MS) {
      logger23.warning(`system.loadFrameTimeout or system.iframeHashTimeout set to lower (${timeout}ms) than the default (${DEFAULT_IFRAME_TIMEOUT_MS}ms). This may result in timeouts.`);
    }
    ...
    invoke(removeHiddenIframe, PerformanceEvents.RemoveHiddenIframe, logger23, performanceClient, correlationId)(iframe);
  });
}
...
var SilentIframeClient = class extends StandardInteractionClient {
  constructor(config, storageImpl, browserCrypto, logger23, eventHandler, navigationClient, apiId, performanceClient, nativeStorageImpl, nativeMessageHandler, correlationId) {
    super(config, storageImpl, browserCrypto, logger23, eventHandler, navigationClient, performanceClient, nativeMessageHandler, correlationId);
    this.apiId = apiId;
    this.nativeStorage = nativeStorageImpl;
  }
...
var TokenCache = class {
  constructor(configuration, storage, logger23, cryptoObj) {
...
var SilentAuthCodeClient = class extends StandardInteractionClient {
  constructor(config, storageImpl, browserCrypto, logger23, eventHandler, navigationClient, apiId, performanceClient, nativeMessageHandler, correlationId) {
    super(config, storageImpl, browserCrypto, logger23, eventHandler, navigationClient, performanceClient, nativeMessageHandler, correlationId);
    this.apiId = apiId;
  }
  /**
...
var NestedAppAuthAdapter = class {
  constructor(clientId, clientCapabilities, crypto, logger23) {
    this.clientId = clientId;
...
var PublicClientApplication = class _PublicClientApplication {
   ...
   */
  setLogger(logger23) {
    this.controller.setLogger(logger23);
  }
  /**
   * Sets the account to use as the active account. If no account is passed to the acquireToken APIs, then MSAL will use this active account.
   * @param account
...
function getPerfMeasurementModule() {
  ...
  try {
    ...
    if (Number(perfEnabled) === 1) {
      return import("./BrowserPerformanceMeasurement-SK5YIJGI.js");
    }
  } catch (e) {
  }
  return void 0;
...
var logger5 = createClientLogger("core-rest-pipeline");
...
function logPolicy(options = {}) {
  var _a3;
  const logger23 = (_a3 = options.logger) !== null && _a3 !== void 0 ? _a3 : logger5.info;
  ...
  return {
    ...
    async sendRequest(request, next) {
      ...
      logger23(`Request: ${sanitizer.sanitize(request)}`);
      const response = await next(request);
      logger23(`Response status code: ${response.status}`);
      logger23(`Headers: ${sanitizer.sanitize(response.headers)}`);
      return response;
    }
  };
}
...
function retryPolicy(strategies, options = { maxRetries: DEFAULT_RETRY_POLICY_COUNT }) {
  const logger23 = options.logger || retryPolicyLogger;
  return {
    ...
    async sendRequest(request, next) {
      ...
      retryRequest: while (true) {
        ...
        try {
          logger23.info(`Retry ${retryCount}: Attempting to send request`, request.requestId);
          response = await next(request);
          logger23.info(`Retry ${retryCount}: Received a response from request`, request.requestId);
        } catch (e) {
          logger23.error(`Retry ${retryCount}: Received an error from request`, request.requestId);
          responseError = e;
        ...
        if ((_a3 = request.abortSignal) === null || _a3 === void 0 ? void 0 : _a3.aborted) {
          logger23.error(`Retry ${retryCount}: Request aborted.`);
          const abortError = new AbortError3();
          throw abortError;
        }
        if (retryCount >= ((_b2 = options.maxRetries) !== null && _b2 !== void 0 ? _b2 : DEFAULT_RETRY_POLICY_COUNT)) {
          logger23.info(`Retry ${retryCount}: Maximum retries reached. Returning the last received response, or throwing the last received error.`);
          if (responseError) {
            throw responseError;
          } else if (response) {
            return response;
        ...
        logger23.info(`Retry ${retryCount}: Processing ${strategies.length} retry strategies.`);
        ...
        if (responseError) {
          logger23.info(`None of the retry strategies could work with the received error. Throwing it.`);
          throw responseError;
        }
        if (response) {
          logger23.info(`None of the retry strategies could work with the received response. Returning it.`);
          return response;
        }
      }
    }
...
function tracingPolicy(options = {}) {
  ...
  return {
    ...
    async sendRequest(request, next) {
      ...
      const { span, tracingContext } = (_b2 = tryCreateSpan(tracingClient2, request, spanAttributes)) !== null && _b2 !== void 0 ? _b2 : {};
...
function bearerTokenAuthenticationPolicy(options) {
  ...
  const logger23 = options.logger || logger5;
...
function checkTenantId(logger23, tenantId) {
  if (!tenantId.match(/^[0-9a-zA-Z-.]+$/)) {
    ...
    logger23.info(formatError("", error));
    throw error;
  }
}
function resolveTenantId(logger23, tenantId, clientId) {
  if (tenantId) {
...
var logger6 = createClientLogger("core-client");
...
var logger7 = credentialLogger("ClientSecretCredential");
  ...
  async getToken(scopes, options = {}) {
    return tracingClient.withSpan(`${this.constructor.name}.getToken`, options, async (newOptions) => {
      ...
      try {
        ...
        logger7.getToken.info(formatError(scopes, err));
        throw err;
      }
    });
  }
...
var logger8 = credentialLogger("EnvironmentCredential");
var EnvironmentCredential = class {
  /**
   * Only available in Node.js
   */
  constructor() {
    logger8.info(formatError("", BrowserNotSupportedError2));
    throw BrowserNotSupportedError2;
  }
  getToken() {
    logger8.getToken.info(formatError("", BrowserNotSupportedError2));
    throw BrowserNotSupportedError2;
  }
};
...
var logger9 = credentialLogger("ClientCertificateCredential");
var ClientCertificateCredential = class {
  /**
   * Only available in Node.js
   */
  constructor() {
    logger9.info(formatError("", BrowserNotSupportedError3));
    throw BrowserNotSupportedError3;
  }
  getToken() {
    logger9.getToken.info(formatError("", BrowserNotSupportedError3));
    throw BrowserNotSupportedError3;
  }
};
...
var logger10 = credentialLogger("ClientAssertionCredential");
var ClientAssertionCredential = class {
  /**
   * Only available in Node.js
   */
  constructor() {
    logger10.info(formatError("", BrowserNotSupportedError4));
    throw BrowserNotSupportedError4;
  }
  getToken() {
    logger10.getToken.info(formatError("", BrowserNotSupportedError4));
    throw BrowserNotSupportedError4;
  }
};
...
var logger11 = credentialLogger("AzureCliCredential");
var AzureCliCredential = class {
  /**
   * Only available in Node.js
   */
  constructor() {
    logger11.info(formatError("", BrowserNotSupportedError5));
    throw BrowserNotSupportedError5;
  }
  getToken() {
    logger11.getToken.info(formatError("", BrowserNotSupportedError5));
    throw BrowserNotSupportedError5;
  }
};
...
var logger12 = credentialLogger("AzureDeveloperCliCredential");
var AzureDeveloperCliCredential = class {
  /**
   * Only available in Node.js
   */
  constructor() {
    logger12.info(formatError("", BrowserNotSupportedError6));
    throw BrowserNotSupportedError6;
  }
  getToken() {
    logger12.getToken.info(formatError("", BrowserNotSupportedError6));
    throw BrowserNotSupportedError6;
  }
};
...
var MSALAuthCode = class extends MsalBrowser {
  /**
...
var logger13 = credentialLogger("InteractiveBrowserCredential");
var InteractiveBrowserCredential = class {
  ...
  constructor(options) {
    if (!(options === null || options === void 0 ? void 0 : options.clientId)) {
      ...
      logger13.info(formatError("", error));
      throw error;
    }
    ...
    const msalOptions = Object.assign(Object.assign({}, options), { tokenCredentialOptions: options, logger: logger13, loginStyle, redirectUri: typeof options.redirectUri === "function" ? options.redirectUri() : options.redirectUri });
...
var logger14 = credentialLogger("ManagedIdentityCredential");
var ManagedIdentityCredential = class {
  constructor() {
    logger14.info(formatError("", BrowserNotSupportedError7));
    throw BrowserNotSupportedError7;
  }
  async getToken() {
    logger14.getToken.info(formatError("", BrowserNotSupportedError7));
    throw BrowserNotSupportedError7;
  }
};
...
var logger15 = credentialLogger("DeviceCodeCredential");
var DeviceCodeCredential = class {
  /**
   * Only available in Node.js
   */
  constructor() {
    logger15.info(formatError("", BrowserNotSupportedError8));
    throw BrowserNotSupportedError8;
  }
  getToken() {
    logger15.getToken.info(formatError("", BrowserNotSupportedError8));
    throw BrowserNotSupportedError8;
  }
};

// node_modules/@azure/identity/dist-esm/src/credentials/azurePipelinesCredential.browser.js
var BrowserNotSupportedError9 = new Error("AzurePipelinesCredential is not supported in the browser.");
var logger16 = credentialLogger("AzurePipelinesCredential");
var AzurePipelinesCredential = class {
  /**
   * Only available in Node.js
   */
  constructor() {
    logger16.info(formatError("", BrowserNotSupportedError9));
    throw BrowserNotSupportedError9;
  }
  getToken() {
    logger16.getToken.info(formatError("", BrowserNotSupportedError9));
    throw BrowserNotSupportedError9;
  }
};
...
var logger17 = credentialLogger("AuthorizationCodeCredential");
var AuthorizationCodeCredential = class {
  constructor() {
    logger17.info(formatError("", BrowserNotSupportedError10));
    throw BrowserNotSupportedError10;
  }
  getToken() {
    logger17.getToken.info(formatError("", BrowserNotSupportedError10));
    throw BrowserNotSupportedError10;
  }
};
...
var UsernamePasswordCredential = class {
  ...
  constructor(tenantIdOrName, clientId, username, password, options) {
    checkTenantId(logger19, tenantIdOrName);
    this.identityClient = new IdentityClient(options);
...
var logger20 = credentialLogger("VisualStudioCodeCredential");
var VisualStudioCodeCredential = class {
  /**
   * Only available in Node.js
   */
  constructor() {
    logger20.info(formatError("", BrowserNotSupportedError12));
    throw BrowserNotSupportedError12;
  }
  getToken() {
    logger20.getToken.info(formatError("", BrowserNotSupportedError12));
    throw BrowserNotSupportedError12;
  }
};
...
var logger21 = credentialLogger(credentialName);
var OnBehalfOfCredential = class {
  /**
   * Only available in Node.js
   */
  constructor() {
    logger21.info(formatError("", BrowserNotSupportedError13));
    throw BrowserNotSupportedError13;
  }
  getToken() {
    logger21.getToken.info(formatError("", BrowserNotSupportedError13));
    throw BrowserNotSupportedError13;
  }
};
...
var logger22 = credentialLogger("WorkloadIdentityCredential");
var WorkloadIdentityCredential = class {
  /**
   * Only available in Node.js
   */
  constructor() {
    logger22.info(formatError("", BrowserNotSupportedError14));
   ...
   */
  getToken() {
    logger22.getToken.info(formatError("", BrowserNotSupportedError14));
    throw BrowserNotSupportedError14;
  }
};
